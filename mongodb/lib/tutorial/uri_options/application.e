...
feature {NONE} -- Initialization
	...
	uri_options_example
		...
		do
			...
			app_name := uri.get_option_as_utf8 ("appname", "DefaultApp")
			print ("Application name: " + if attached app_name as l_app then l_app else "" end + "%N")

				-- Get integer (32-bit) option
			max_pool_size := uri.get_option_as_int32 ("maxPoolSize", 10)
			print ("Max pool size: " + max_pool_size.out + "%N")

				-- Get integer (64-bit) option
			max_staleness := uri.get_option_as_int64 ("maxStalenessSeconds", 60)
			print ("Max staleness seconds: " + max_staleness.out + "%N")

				-- Get boolean option
			retry_writes := uri.get_option_as_bool ("retryWrites", False)
			...
			print ("Non-existent option (string): " +
				if attached uri.get_option_as_utf8 ("nonexistent", "fallback") as l_res then l_res else "" end + "%N")
			print ("Non-existent option (int32): " +
				uri.get_option_as_int32 ("nonexistent", -1).out + "%N")
			print ("Non-existent option (int64): " +
				uri.get_option_as_int64 ("nonexistent", -1).out + "%N")
			print ("Non-existent option (bool): " +
				uri.get_option_as_bool ("nonexistent", True).out + "%N")
